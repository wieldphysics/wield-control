...
def test_hard_soft_modes(tpath_join, dprint, capture):
    ...
    # the w keyword), in the s-domain (with the s keyword), or in the z-domain
    # (with the z keyword)
    ...
    # Input/output degrees of freedom/test points are defined by dictionaries
    # specifying the row/column indices to which they correspond. They can have
    # different names but are the same in this simple example.
    dofs = {'etm': 0, 'itm': 1}
    ...
    fback_ss = free_mimo.feedback(connections)
