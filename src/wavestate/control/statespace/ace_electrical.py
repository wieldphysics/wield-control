...
def zpkACE(
    ...
    for idx in range(len(syslist) - 1):
        ace.bind_equal({"sys{}.O".format(idx), "sys{}.I".format(idx + 1)}, constr = 's{}{}'.format(idx, idx+1))
...
def op_amp(Gbw = 1e7):
    ace = ACE.ACE()
    ace.insert(
        zpkACE(Pr = [-1*pi2, ], k = Gbw),
        cmn = 'Vgain'
    )
    ace.insert(
        zpkACE(Pr = [-1*pi2, ], Zr = [-100*pi2, ], k = 4e-9 * 100),
        cmn = 'Vnoise'
    )
    ace.insert(
        zpkACE(Pr = [-1*pi2, ], Zr = [-30*pi2, ], k = 10e-12 * 100),
        cmn = 'Inoise'
    )
