...
def test_lqe1(pprint, test_trigger, tpath_join, tpath_preclear, plot):
    ...
    ax0B = axB.ax0.twinx()
    ax0B.loglog(F_Hz, abs(w_xfer), color = 'black', ls = '--', lw = 1, label = 'weighting function')
    ax0B.loglog(F_Hz, abs(w_xfer2), color = 'purple', ls = '--', lw = 1, label = 'weighting function')
    axB.ax0.loglog(F_Hz, abs(s1_xfer), label = 'Sensor 1, good but tilt-contaminated', lw = 2)
    ...
    axB.ax0.loglog(F_Hz, abs(xfer_opt), color = 'magenta', lw=2, label = 'aspirational optimum')
    ...
    ssB = ace.statespace(inputs = ['wg1.I'], outputs = ['ws1.I', 'ws2.I', 'g1.I'], Dreduce = False)
    ...
    ssB = ace.statespace(inputs = ['wg1.I'], outputs = ['ws1.I', 'ws2.I', 'g1.O'], Dreduce = True)
    ACE.printSSBnz(ssB)
    ...
    def trigger(fail, plot):
        ...
        axB.ax1.semilogx(F_Hz, np.angle(xfer_g1, deg = True), label = 'Seismic Background', lw = 2)
        axB.ax0.loglog(F_Hz, abs(xfer_s1), label = 'Sensor 1, good but tilt-contaminated', lw = 2)
        axB.ax1.semilogx(F_Hz, np.angle(xfer_s1, deg = True), label = 'Sensor 1, good but tilt-contaminated', lw = 2)
        axB.ax0.loglog(F_Hz, abs(xfer_s2), label = 'Sensor 2, bad but broadband', lw = 2)
        axB.ax1.semilogx(F_Hz, np.angle(xfer_s2, deg = True), label = 'Sensor 2, bad but broadband', lw = 2)
        axB.ax0.loglog(F_Hz, abs(xfer_opt), color = 'magenta', lw=2, label = 'aspirational optimum')
        axB.ax0.set_xlabel('Frequency [Hz]')
        axB.ax0.set_ylabel('Seismic Acceleration-like units')
        axB.ax0.legend()
...
def test_lqe2(pprint, test_trigger, tpath_join, tpath_preclear, plot):
    A = np.array([
        ...
        [3.94784176e+00, -1.25663706e+00,  0.00000000e+00, 0.00000000e+00],
        [0.00000000e+00,  9.99900000e+01,  0.00000000e+00, -1.00000000e+02],
        [0.00000000e+00,  1.24407069e+00,  3.94784176e-03, -1.25663706e+00]])
    B = np.array([
        [1.50000000e-06],
        [3.76991118e-06],
        [9.99900000e-05],
