...
class StateSpaceDense(object):
    ...
    #    other.C = np.copy(self.C)
    #    other.D = np.copy(self.D)
    #    other.E = np.copy(self.E)
    #    other.inputs = copy.deepcopy(self.inputs, memo)
    #    other.output = copy.deepcopy(self.output, memo)
    #    other.states = copy.deepcopy(self.states, memo)
    #    other.constr = copy.deepcopy(self.constr, memo)
    #    return other
    ...
    def percolate_names(
        ...
        idx_drops = []
        for name in names:
            idx = group.name2idx[name]
            R1 = group.idx2N[idx]
            R2 = group.idx2N[idx + 1]
            ranges.append((R1, R2))
            idx_drops.append(idx)

        ranges_sorted = []
        dropsort = np.argsort(idx_drops)
        ...
        if keep:
            if collect is None:
                group.idx2name.extend(keepnames[::-1])
                Ntot = 0
